      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <form action={createEmployee} className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          ...
          <div className="md:col-span-2 xl:col-span-4">
            <button type="submit" className="rounded-lg bg-slate-900 px-5 py-2.5 text-sm font-bold text-white hover:bg-slate-800">
              حفظ الموظف
            </button>
          </div>
        </form>
      ...
      {employees.length === 0 ? (
        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <p className="mt-2 text-sm leading-7 text-slate-600">
            بيانات Excel تحدد أماكن المواد، ويمكن الآن إضافة الموظفين وربطهم بهذه المواقع دون إعادة إدخال أي مادة.
          </p>
        </section>
      ) : (
              ...
              <article key={employee.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                  ...
                  <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-700">
                    {currentLocation?.name ?? "غير مرتبط بموقع"}
                  </span>
                </div>
                ...
                <form action={createCustodyDraftFromRoom} className="mt-3">
                  <input type="hidden" name="employeeId" value={employee.id} />
                  <button
                    disabled={!currentLocation}
                    className="w-full rounded-lg bg-slate-900 px-4 py-2.5 text-sm font-bold text-white enabled:hover:bg-slate-800 disabled:cursor-not-allowed disabled:bg-slate-300"
                  >
                    إنشاء مسودة سند عهدة من محتويات الغرفة
                  </button>
                </form>
              </article>
            );
