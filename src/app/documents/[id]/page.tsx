...
export default async function DocumentPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    return (
      <div className="space-y-6">
        <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
          ...
          <span className={`w-fit rounded-full px-3 py-1 text-xs font-bold ${statusBadge(document.status)}`}>
        ...
        <form action={saveCustodyDraft} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
                ...
                {document.lines.map((line) => (
                  <tr key={line.id}>
                    ...
                    <td className="px-4 py-3 text-center">
                      {isDraft ? (
                        <input type="number" min={1} max={line.quantity} name={`quantity:${line.id}`} defaultValue={line.quantity} className="w-24 rounded-lg border border-slate-300 px-2 py-1.5 text-center" />
          ...
          {isDraft ? (
            <div className="border-t border-slate-200 p-4">
              <button className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-bold hover:bg-slate-50">حفظ تعديلات المسودة</button>
            </div>
          ) : null}
        </form>
          ...
          {isDraft ? (
            <>
              <form action={postCustodyDocument}>
                <input type="hidden" name="documentId" value={document.id} />
                <button className="w-full rounded-lg bg-emerald-700 px-5 py-2.5 text-sm font-bold text-white hover:bg-emerald-800 sm:w-auto">اعتماد السند وتثبيت العهدة</button>
              </form>
            ...
            <Link href={`/documents/${document.id}/print`} className="rounded-lg bg-slate-900 px-5 py-2.5 text-center text-sm font-bold text-white hover:bg-slate-800">معاينة وطباعة سند العهدة</Link>
          )}
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          <p className="text-sm text-slate-500">{isReceipt ? "مستند إدخال لوازم" : "مستند إخراج لوازم"}</p>
          <h2 className="mt-1 text-2xl font-bold text-slate-900">{document.documentNo ? `رقم ${document.documentNo}` : "بدون رقم مستند"}</h2>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="grid gap-4 text-sm md:grid-cols-2 lg:grid-cols-4">
          <div><span className="text-slate-500">{isReceipt ? "موقع الاستلام" : "موقع الإخراج"}</span><div className="mt-1 font-bold text-slate-900">{isReceipt ? document.toLocation?.name : document.fromLocation?.name}</div></div>
          <div><span className="text-slate-500">{isReceipt ? "المورد / الجهة" : "الجهة / الشخص المصروف له"}</span><div className="mt-1 font-bold text-slate-900">{document.counterparty?.name ?? "—"}</div></div>
        ...
        {isReceipt && document.receiptDetail ? (
          <div className="mt-5 grid gap-4 border-t border-slate-100 pt-5 text-sm md:grid-cols-2 lg:grid-cols-5">
            <div><span className="text-slate-500">رقم الفاتورة</span><div className="mt-1 font-medium">{document.receiptDetail.invoiceNo ?? "—"}</div></div>
            <div><span className="text-slate-500">تاريخ الفاتورة</span><div className="mt-1 font-medium">{document.receiptDetail.invoiceDate?.toLocaleDateString("ar-JO") ?? "—"}</div></div>
            <div><span className="text-slate-500">لجنة الاستلام</span><div className="mt-1 font-medium">{document.receiptDetail.receivingCommitteeDate?.toLocaleDateString("ar-JO") ?? "—"}</div></div>
            <div><span className="text-slate-500">مرجع التقرير</span><div className="mt-1 font-medium">{document.receiptDetail.reportReference ?? "—"}</div></div>
            <div><span className="text-slate-500">أمر الشراء</span><div className="mt-1 font-medium">{document.receiptDetail.purchaseOrderNo ?? "—"}</div></div>
          </div>
        ) : null}
      </section>
          ...
          <table className="min-w-[850px] w-full text-sm">
            <thead className="bg-slate-50 text-slate-600">
              <tr>
                ...
                <th className="px-4 py-3 text-center">الكمية</th>
                {isReceipt ? <><th className="px-4 py-3 text-center">سعر الوحدة</th><th className="px-4 py-3 text-center">القيمة</th></> : null}
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {document.lines.map((line) => (
                <tr key={line.id}>
                  ...
                  {isReceipt ? <><td className="px-4 py-3 text-center">{money(line.unitPriceFils)}</td><td className="px-4 py-3 text-center font-bold">{money(line.totalValueFils)}</td></> : null}
                </tr>
              ))}
            </tbody>
            {isReceipt ? (
              <tfoot className="bg-slate-50 font-bold">
                <tr><td colSpan={6} className="px-4 py-3 text-left">الإجمالي (د.أ)</td><td className="px-4 py-3 text-center">{money(totalFils)}</td></tr>
              </tfoot>
            ) : null}
          </table>
        </div>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap">
        {isDraft ? (
          <>
            <form action={postStockDocument}>
              ...
              <button className="w-full rounded-lg bg-emerald-700 px-5 py-2.5 text-sm font-bold text-white hover:bg-emerald-800 sm:w-auto">
                {isReceipt ? "اعتماد المستند وإضافة الكميات للمخزون" : "اعتماد المستند وخصم الكميات من المخزون"}
              </button>
            </form>
          ...
          <Link href={`/documents/${document.id}/print`} className="rounded-lg bg-slate-900 px-5 py-2.5 text-center text-sm font-bold text-white hover:bg-slate-800">معاينة وطباعة المستند</Link>
        )}
        <Link href="/documents" className="rounded-lg border border-slate-300 px-5 py-2.5 text-center text-sm font-medium hover:bg-slate-50">العودة للمستندات</Link>
      </div>
    </div>
